...
{
	this.setInventoryName("Solar Death Ray Tower\nEnergy: "+this.get_u16("power")+"/"+MAX_ENERGY);
	if (isClient())
	{
	...
	if (getGameTime()%450==0 && this.get_u16("power") > 0)
	{
		this.add_u16("power", -1);
	}
}
...
void GetButtonsFor(CBlob@ this, CBlob@ caller)
{
	...
	if (carried != null && carried.getName() == "mat_battery")
	{
		...
		CButton@ button = caller.CreateGenericButton(23, Vec2f(0, 0), this, this.getCommandID("refill"), "Add 50 batteries to Beam Tower", params);
	}
}
...
void Shoot(CBlob@ this, f32 power, Vec2f dir)
{
	...
	{	
		if (power > 0.50f)
		{
					...
					if (blob !is null)
					{
						if (!blob.hasTag("invincible") && !blob.hasTag("no beamtower damage"))
						{
							if (isServer())
							{
								...
								this.server_Hit(blob, blob.getPosition(), dir, damage*1.5, HittersTC::plasma, true);
							}
			...
			u8 rand = XORRandom(this.get_u16("power")/30)+3;
			if (rand >= this.get_u16("power"))
			{
				rand = 0;
				this.set_u16("power", 0);
			}
			this.add_u16("power", -rand);
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	{		
		if (isServer())
		{
			...
			f32 power = this.get_u16("power")/(17+XORRandom(7));
