...
void GetButtonsFor(CBlob@ this, CBlob@ caller) //Mutate button
{
	if (this.isOverlapping(caller))
	{
		...
		if (carried != null && (carried.getName() == "mat_mithrilingot"))
		{
			...
			CButton@ button = caller.CreateGenericButton(23, Vec2f(0, -6), this, this.getCommandID("upgrade"), "Upgrade Reactor for 8 Mithril ingots", params);
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params) //Mutate command
{
	if (cmd == this.getCommandID("upgrade"))
	{
		...
		if (caller !is null && this.get_f32("upgrade") <= 28000.0f)
		{
			CBlob@ carried = caller.getCarriedBlob();
			if (carried !is null && (carried.getName() == "mat_mithrilingot" || carried.getName() == "mat_steelingot"))
			{
				if (carried.getQuantity() >= 8)
				{
					
					int remain = carried.getQuantity() - 8;
					if (remain > 0)
					{
