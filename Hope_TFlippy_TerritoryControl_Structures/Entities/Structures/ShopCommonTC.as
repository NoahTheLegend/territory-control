shared class ShopItemTC
{
	...
	ShopItemTC(CBitStream @bt)
	{
		if (!Unserialise(bt))
		{
			warn("Error unserializing ShopItemTC");
			name = "";
		}
	}
...
const string SHOP_ARRAY_TC = "shop array";
const string SHOP_AUTOCLOSE_TC = "auto close menu";

//adding a item to a blobs list of items

ShopItemTC@ addShopItemTC(CBlob@ this, const string &in name, const string &in iconName, const string &in blobName, const string &in description)
{
	return addShopItemTC(this, SHOP_ARRAY_TC
, name, iconName, blobName, description, false, false);
}

ShopItemTC@ addShopItemTC(CBlob@ this, const string &in name, const string &in iconName, const string &in blobName, const string &in description, bool spawnToInventory)
{
	return addShopItemTC(this, SHOP_ARRAY_TC
, name, iconName, blobName, description, spawnToInventory, false);
}

ShopItemTC@ addShopItemTC(CBlob@ this, const string &in name, const string &in  iconName, const string &in  blobName, const string &in description, bool spawnToInventory, bool spawnInCrate)
{
	return addShopItemTC(this, SHOP_ARRAY_TC
, name, iconName, blobName, description, spawnToInventory, spawnInCrate);
}

ShopItemTC@ addShopItemTC(CBlob@ this, const string &in shopArray, const string &in name, const string &in iconName, const string &in blobName, const string &in description, bool spawnToInventory, bool spawnInCrate)
{
...
void removeShopItemTC(CBlob@ this, const string &in name, const string &in iconName, const string &in blobName, const string &in description)
{
	removeShopItemTC(this, SHOP_ARRAY_TC, name, iconName, blobName, description, false, false);
}

void removeShopItemTC(CBlob@ this, const string &in shopArray, const string &in name, const string &in iconName, const string &in blobName, const string &in description, bool spawnToInventory, bool spawnInCrate)
{
	...
	if (this.get(shopArray, @shop_items))
	{
		for (uint i = 0 ; i < shop_items.length; i++)
		{
			...
			{
				shop_items.removeAt(i);
			}
		}
	}
}

void removeOfflineShopItemTC(CBlob@ this, const string &in shopArray)
{
	// check if item exists and belongs to an offline player
	if (!this.exists(shopArray))
	{
		ShopItemTC[] items;
		this.set(shopArray, items);
	...
	if (this.get(shopArray, @shop_items))
	{
		for (uint i = 0 ; i < shop_items.length; i++)
		{
			...
			for (u8 j = 0; j < getPlayersCount(); j++)
			{
				...
				if (item.blobName == p.getCharacterName())
				{
					isOnline = true;
				}
			}
...
void ShopSendCreateDataTC(CBlob@ this, CBitStream@ stream, const string &in shopArray)
{
	ShopItemTC[]@ items;
...
bool ShopReceiveCreateDataTC(CBlob@ this, CBitStream@ stream, const string &in shopArray)
{
	u8 itemsCount;
	...
	{
		ShopItemTC@ item = addShopItemTC(this, shopArray, "", "", "", "", false, false);
		if (item !is null)
		{
