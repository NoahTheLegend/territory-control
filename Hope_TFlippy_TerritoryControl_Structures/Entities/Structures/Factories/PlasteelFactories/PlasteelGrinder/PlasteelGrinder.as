...
void onInit(CSprite@ this)
{
	...
	CSpriteLayer@ chop_left = this.addSpriteLayer("chop_left", "PlasteelGrinder_Saw.png", 16, 16);
	...
	CSpriteLayer@ chop_right = this.addSpriteLayer("chop_right", "PlasteelGrinder_Saw.png", 16, 16);
	...
	if (getMap().getBlobsInRadius(this.getPosition()+Vec2f(0,-2), 6.4f, @blobs))
	{
		...
		for (uint i = 0; i < length; i++)
		{
			...
			if (canSaw(this, blob))
			{
				...
				{ 
					this.server_Hit(blob, blob.getPosition(), Vec2f(0, -2), 4.00f, Hitters::saw, true); 
				}
			}
	...
	switch(hash)
	{
		...
		case 575725963://mat_stone
		{
			...
			if (isServer())
			{
				...
				MakeMat(this, this.getPosition(), "mat_iron", 		XORRandom(quantity * 0.16f));
				MakeMat(this, this.getPosition(), "mat_sulphur", 	XORRandom(quantity * 0.20f));
