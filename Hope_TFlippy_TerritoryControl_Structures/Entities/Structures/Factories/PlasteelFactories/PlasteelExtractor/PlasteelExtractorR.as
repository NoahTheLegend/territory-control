...
void onInit(CSprite@ this)
{
	...
	CSpriteLayer@ gear = this.addSpriteLayer("gear", "P_extractor_cog.png" , 16, 16, this.getBlob().getTeamNum(), this.getBlob().getSkinNum());
...
void onTick(CBlob@ this)
{
	...
	this.getCurrentScript().tickFrequency = 15.0f / (this.exists("gyromat_acceleration") ? this.get_f32("gyromat_acceleration") : 1);
	...
	if (isServer())
	{
		...
		//if (getMap().getBlobsInBox(this.getPosition() + Vec2f(-40, -40), this.getPosition() + Vec2f(40, 40), @blobs))
		{
			for (uint i = 0; i < blobs.length; i++)
			{
				...
				if (b.hasTag("ignore extractor")) continue;
				if (b.getInventory() !is null && b.hasTag("extractable")){
			...
			for (uint i = 0; i < blobs.length; i++)
			{
				...
				if (b.getInventory() !is null && b.hasTag("extractable")){
					...
					if(layer is null){
						...
						CSpriteLayer@ gear = sprite.addSpriteLayer("connection_"+b.getNetworkID(),"PlasteelExtractorPipeR.png",dis,6);
						if(gear !is null){
