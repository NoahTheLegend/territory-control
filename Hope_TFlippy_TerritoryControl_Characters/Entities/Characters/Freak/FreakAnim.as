...
{
	addRunnerTextures(this, "freak", "Freak");
...
{
	ensureCorrectRunnerTexture(this, "freak", "Freak");
}
...
void onTick(CSprite@ this)
{
	...
	FreakInfo@ freak;
	if (!blob.get("FreakInfo", @freak)) return;
	...
	// animations
	bool grabbed = freak.state == FreakStates::grabbed;

	blob.Untag("attack head");
	blob.Untag("dead head");

	if (freak.state != FreakStates::fatality) this.SetOffset(Vec2f());

	if (freak.state == FreakStates::fatality)
	{
		if (freak.prevState != freak.state) this.SetFacingLeft(false);
