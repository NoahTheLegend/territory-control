...
const f32 arrowInsaneSpeed = 18.0f;
//maximum is 15 as of 22/11/12 (see ArcherCommon.as)

const f32 ARROW_PUSH_FORCE = 6.0f;
const f32 SPECIAL_HIT_SCALE = 3.0f; //special hit on food items to shoot to team-mates
...
bool specialArrowHit(CBlob@ blob)
{
	...
	return (bname == "fishy" && blob.hasTag("dead") || bname == "food"
		|| bname == "steak" || bname == "grain" || bname == "chair"); //no egg because logic
}
...
f32 getArrowDamage(CBlob@ this, f32 vellen = -1.0f)
{
	...
	if (vellen >= arrowInsaneSpeed)
	{
		this.get_u8("arrow type") == ArrowType::fire ? dmg = 0.5f : dmg = 2.0f;
	}
	else if (vellen >= arrowFastSpeed)
	{
		this.get_u8("arrow type") == ArrowType::fire ? dmg = 0.25f : dmg = 1.5f;
	}
	else if (vellen >= arrowMediumSpeed)
	{
		this.get_u8("arrow type") == ArrowType::fire ? dmg = 0.1f : dmg = 1.0f;
	}
	else
		this.get_u8("arrow type") == ArrowType::fire ? dmg = 0.1f : dmg = 0.5f;
