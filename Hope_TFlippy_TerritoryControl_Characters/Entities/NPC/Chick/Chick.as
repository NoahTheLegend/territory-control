...
void onInit(CSprite@ this)
{
	...
	this.getBlob().set_u32("grow", getGameTime() + ((XORRandom(61)+120)*30)); // from 2 to 3 minutes
}
...
void onTick(CSprite@ this)
{
	...
	else
	{
		...
		this.PlaySound("/ScaredChicken", 0.85f, 1.33f);
	}
}
...
{
	if (isServer() && this.get_u32("grow") <= getGameTime())
	{
		this.Tag("dead");
		this.server_Die();
	...
	{
		this.getSprite().PlaySound("/Pluck", 0.85f, 1.33f);
		g_lastSoundPlayedTime =  getGameTime();
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	...
	{
		this.getSprite().PlaySound("/ScaredChicken", 0.85f, 1.33f);
		g_lastSoundPlayedTime = getGameTime();
	}
}
