...
const Vec2f miniGun_offset = Vec2f(-40,10);
const Vec2f rocketminiGun_offset = Vec2f(-11,5);
...
const u32 shootDelay = 1.7; // Ticks
const u32 shootDelayRocket = 10; // Ticks
const f32 damage = 4.0f;
const int maxRocketStack = 100;
const int maxAmmoStack = 5000;
...
void onInit(CBlob@ this)
{
	...
	this.set_u16("ammoCount", 5000);
	this.set_u16("rocketCount", 100);
...
{
	//Add minigun
	CSpriteLayer@ mini = this.addSpriteLayer("minigun", "Mason_Copter_Gun.png", 16, 16);
	if (mini !is null)
	{
	...
	if (flash !is null)
	{
		...
		flash.SetOffset(Vec2f(-56,8));
		flash.SetVisible(false);
		//flash.setRenderStyle(RenderStyle::light);
	}
	
	//Add blade
	CSpriteLayer@ blade = this.addSpriteLayer("blade", "Mason_Copter_Blade.png", 67, 8);
	if (blade !is null)
	{
	...
	CSpriteLayer@ tailrotor = this.addSpriteLayer("tailrotor", "Mason_Copter_TailRotor.png", 16, 16);
	if (tailrotor !is null)
	{
		...
		tailrotor.SetOffset(Vec2f(36, -10));
		tailrotor.SetRelativeZ(50.0f);
		tailrotor.SetVisible(true);
	}

	//Add launcher
	CSpriteLayer@ rocket = this.addSpriteLayer("rocketlauncher", "Mason_Copter_Launcher.png", 16, 16);
	if (rocket !is null)
	{
	...
	//Add copter wing
	CSpriteLayer@ wing = this.addSpriteLayer("wing", "Mason_Copter_Wing.png", 13, 5);
	if (wing !is null)
	{
	...
	//Add balkenkreuz
	CSpriteLayer@ balkenkreuz = this.addSpriteLayer("balkenkreuz", "Mason_Copter_Balkenkreuz.png", 10, 10);
	...
	this.SetEmitSound("AAAA_mason.ogg");
	...
	if (caller.getTeamNum() == this.getTeamNum())
	{
		...
		}
		{
			...
			if (carried !is null && this.get_u16("rocketCount") < maxRocketStack)
			{
				...
				caller.CreateGenericButton("$masonmissile$", Vec2f(3, 2), this, this.getCommandID("addRocket"), "Load Missiles\n(" + this.get_u16("rocketCount") + " / " + maxRocketStack + ")", params);
			}
		}
		{
			CBitStream params;
	...
	else if (cmd == this.getCommandID("play_music"))
	{
		...
		if(carried !is null){
			...
			if (this.exists("bladeid"))
			{
				...
				if (blade !is null)
				{
					blade.getSprite().SetEmitSound("Disc_MountainKing.ogg");
					blade.getSprite().SetEmitSoundPaused(false);
					blade.getSprite().SetEmitSoundVolume(1.5);
				}
			}
			carried.server_Die();
		}
...
void shootRocket(CBlob@ this, f32 angle, CBlob@ target, Vec2f gunPos)
{
	...
	if (isServer())
	{
		CBlob@ m = server_CreateBlobNoInit("masonmissile");
