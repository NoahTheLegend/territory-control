...
void onTick(CBlob@ this)
{
	...
	if (isClient())
	{
		...
		{
			this.getSprite().PlaySound("build_wall" + XORRandom(1) + ".ogg", 10.5f, 1.00f);
	...
	{
		moveVars.walkFactor *= 2.70f;
		moveVars.jumpFactor *= 4.20f;
		moveVars.swimspeed *= 6.00f;
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	if (!this.hasTag("dead"))
	{
		if (isClient())
		{
			...
			{
				this.getSprite().PlaySound("build_wall" + XORRandom(2) + ".ogg", 2, 1.0f);
	...
	if (this.getMap().getBlobsInRadius(worldPoint + (dir * Maths::Min(len, 16)), 16, @blobsInRadius))
	{
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			...
			if (hitBlob !is null && hitBlob !is this)
			{
				...
				{
					this.getSprite().PlaySound("build_wall" + XORRandom(3), 10.1f, 1.00f);
