...
void onInit(CBlob@ this)
{
	...
	sprite.SetEmitSound("build_wall.ogg");
...
void onTick(CBlob@ this)
{
	...
	if(isClient())
	{
		MakeParticle(this, "MasonNanobot.png");
	...
	else
	{
		...
		if (target !is null)
		{
			if (target.exists("nanobot_netid") && target.get_u16("nanobot_netid") == this.getNetworkID() && !target.hasTag("nanobot_ignore"))
			{
				...
				if (this.get_f32("fill") >= reproduce_threshold)
				{
					if (isServer())
					{
						server_CreateBlob("masonnanobot", -1, pos);
					}
					
					if (isClient())
					{
						this.getSprite().PlaySound("build_wall", 1.00f, 1.00f);
					}
		...
		else if (XORRandom(50) == 0)
		{
			...
			if (target !is null) 
			{
				...
				if (isClient()) this.getSprite().PlaySound("build_wall", 1.00f, 1.00f);
			}
			else
			{
				if (isClient()) this.getSprite().PlaySound("build_wall", 1.00f, 1.00f);
		...
		case 0:
		{
			if (tick % 3 == 0)
			{
				...
				if (isClient())
				{
					if (hit)
					{
						this.getSprite().PlaySound("build_wall.ogg", 0.50f, 0.25f + XORRandom(100) * 0.01f);
					}
				}
			}
		}
...
{
	this.getSprite().PlaySound("/build_wall.ogg", 0.75f, 0.50f + XORRandom(100) * 0.01f);
	this.getSprite().Gib();
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	return remote is null ? damage : 0;
}
