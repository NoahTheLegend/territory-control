...
void onInit(CBlob@ this)
{
	...
	this.addCommandID("change_drilling_type");
	this.set_bool("can_hit_team_blobs", false);
}
...
void onTick(CBlob@ this)
{
	...
	if (this.isAttached())
	{
		...
		if (holder.getName() != "builder" && holder.getName() != "engineer" && holder.getName() != "susengineer" && holder.getName() != "advancedengineer" && holder.getName() != "adminbuilder" && holder.getName() != "exosuit" && holder.getName() != "robosuit" && holder.getName() != "freak")
		{
				...
				if (map.getHitInfosFromArc((this.getPosition() - attackVel), -attackVel.Angle(), 30, distance, this, true, @hitInfos))
				{
					...
					for (uint i = 0; i < hitInfos.length; i++)
					{
						...
						if (b !is null) // blob
						{
							...
							if (b.getTeamNum() == holder.getTeamNum() && b.hasTag("flesh")) continue;
...
void GetButtonsFor(CBlob@ this, CBlob@ caller)
{
	...
	{
		bool state = this.get_bool("can_hit_team_blobs");
		caller.CreateGenericButton((state ? "$no_team_drilling$" : "$team_drilling$"), Vec2f(0, 0), this, this.getCommandID("change_drilling_type"), getTranslatedString(state ?
		"Make it stop damaging team blobs.." :
		"Make it capable of drilling team blobs!"));
	}
}

void onCommand(CBlob@ this, u8 cmd, CBitStream@ params)
{
	if (cmd == this.getCommandID("change_drilling_type"))
	{
		this.set_bool("can_hit_team_blobs", !this.get_bool("can_hit_team_blobs"));
	}
}
...
void onRender(CSprite@ this)
{
	...
	if (holderBlob.getName() != "builder" && holderBlob.getName() != "engineer" && holderBlob.getName() != "susengineer" && holderBlob.getName() != "exosuit" && holderBlob.getName() != "robosuit" && holderBlob.getName() != "advancedengineer" && holderBlob.getName() != "adminbuilder" && holderBlob.getName() != "freak")
	{
