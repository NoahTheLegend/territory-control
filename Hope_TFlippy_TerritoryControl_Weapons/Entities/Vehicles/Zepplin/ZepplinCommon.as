...
void drawFuelCount(CBlob@ this)
{
	...
	GUI::SetFont("menu");
	GUI::DrawTextCentered(reqsText, this.getInterpolatedScreenPos() + Vec2f(0, 120), color_white);
...
void GetButtonsFor(CBlob@ this, CBlob@ caller)
{
	...
	if (carried !is null && this.get_f32("fuel_count") < this.get_f32("max_fuel"))
	{
		...
		if (isValid)
		{
			...
			CButton@ button = caller.CreateGenericButton("$" + fuel_name + "$", Vec2f(0, 5), this, this.getCommandID("load_fuel"), "Load " + carried.getInventoryName() + "\n(" + this.get_f32("fuel_count") + " / " + this.get_f32("max_fuel") + ")", params);
		}
	}
}
...
void onRender(CSprite@ this)
{
	...
	if (fuel <= 0 && mouseOnBlob)
	{
		...
		GUI::SetFont("menu");
		GUI::DrawTextCentered("Requires fuel!", Vec2f(pos.x, pos.y + 140 + Maths::Sin(getGameTime() / 5.0f) * 5.0f), SColor(255, 255, 55, 55)); //85
		GUI::DrawTextCentered("(Wood, Coal, Methane, Fuel or Oil)", Vec2f(pos.x, pos.y + 160 + Maths::Sin(getGameTime() / 5.0f) * 5.0f), SColor(255, 255, 55, 55)); //105
	}
}
