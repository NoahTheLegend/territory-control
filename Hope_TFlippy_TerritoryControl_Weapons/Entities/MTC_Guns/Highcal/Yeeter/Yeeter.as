...
void onInit(CBlob@ this)
{
	...
	//Bullet
	settings.B_PER_SHOT = 30; //Shots per bullet | CHANGE B_SPREAD, otherwise both bullets will come out together
	...
	settings.B_DAMAGE = 0.1f; //1 is 1 heart
	settings.B_TYPE = HittersTC::electric; //Type of bullet the gun shoots | hitter

	//Recoil
	settings.G_RECOIL = -100; //0 is default, adds recoil aiming up
	...
	//Sound
	settings.FIRE_SOUND = "Bang.ogg"; //Sound when shooting
	settings.RELOAD_SOUND = "FugerReload.ogg"; //Sound when reloading

	//Offset
	settings.MUZZLE_OFFSET = Vec2f(-8, -1); //Where the muzzle flash appears

	this.set("gun_settings", @settings);

	//Custom
	this.set_f32("CustomShootVolume", 8.0f);
	this.Tag("CustomSemiAuto");
	this.set_u8("CustomPenetration", 0);
}
