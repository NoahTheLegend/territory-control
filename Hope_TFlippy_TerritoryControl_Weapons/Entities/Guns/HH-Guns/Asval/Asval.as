...
void onInit(CBlob@ this)
{
	...
	settings.RELOAD_TIME = 65; //Time it takes to reload (in ticks)
	...
	settings.B_GRAV = Vec2f(0, 0.04); //Bullet gravity drop
	...
	settings.B_DAMAGE = 1.9f; //1 is 1 heart
	...
	settings.G_BACK_T = 8; //Should we recoil the arm back time? (aim goes up, then back down with this, if > 0, how long should it last)

	//Sound
	settings.FIRE_SOUND = "AsvalFire.ogg"; //Sound when shooting
	settings.RELOAD_SOUND = "AsvalReload.ogg"; //Sound when reloading
	...
	this.Tag("SMG");
	this.set_f32("CustomShootVolume", 5.0f);
}
