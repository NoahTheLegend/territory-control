...
void onTick(CBlob@ this)
{
	if (this.get_u32("fuel_timer") > getGameTime())
	{
		...
		if (isServer())
		{
			...
			for (int i = 0; i < 4; i++)
			{
				Vec2f pos = this.getPosition() - (dir * 16) + Vec2f(XORRandom(16) - 8, XORRandom(16) - 8);
				TileType tile = map.getTile(pos).type;
				if (isTileReinforcedConcrete(tile) || (tile >= 525 && tile <= 536) || isTilePlasteel(tile)) this.server_SetHealth(this.getHealth()-0.75);
				if (this.getHealth() <= 0.0) this.server_Die();
			...
			if (this.getMap().getBlobsInRadius(this.getPosition() - (dir * 4), 8, @blobs))
			{
				for (uint i = 0; i < blobs.length; i++)
				{
					...
					{
						if (blob.getName() == "iron_door" || blob.getName() == "plasteel_door") this.server_Die();
...
void DoExplosion(CBlob@ this, Vec2f velocity)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		LinearExplosion(this, Vec2f(dir.x * jitter.x, dir.y * jitter.y), 8.0f + XORRandom(16), 5.0f, 4, 3.0f, Hitters::explosion);
	}
