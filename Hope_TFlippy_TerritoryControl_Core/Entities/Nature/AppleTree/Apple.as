...
const u16 APPLE_BASE_GROW_TIME = 1500;
const u16 additional_time = 300;
...
void onTick(CBlob@ this)
{
	...
	this.exists("additional grow time") ? this.get_u16("additional grow time") : 0;
	u32 growth_delay = this.get_u32("growth_delay");

	if (this.getTickSinceCreated() > APPLE_BASE_GROW_TIME + 100 + additional_time
		&& growth_delay < getGameTime() && s.getFrame() != 0)  // full apple - stop ticking now
	{
		anim.SetFrameIndex(0);
		this.Untag("apple growth");
		this.AddScript("Eatable.as");
	}
	else if (this.getTickSinceCreated() > APPLE_BASE_GROW_TIME + 50 + additional_time
		&& growth_delay < getGameTime() && s.getFrame() != 2)  // medium apple
	{
		anim.SetFrameIndex(2);
		this.set_u32("growth_delay", getGameTime()+additional_time);
	}
	else if (this.getTickSinceCreated() > APPLE_BASE_GROW_TIME + additional_time
		&& growth_delay < getGameTime() && s.getFrame() != 1) // small apple
	{
