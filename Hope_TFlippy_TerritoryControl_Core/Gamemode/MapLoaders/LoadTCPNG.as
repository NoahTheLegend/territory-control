...
namespace tc_colors
{
	enum color
	{
		...
		color_oilrig_neutral = 0xff1450ff,
		...
		color_bricks = 0xff792a12,
		color_titanium = 0xff767f92,
...
class TCPNGLoader : PNGLoader
{
	...
	void handlePixel(const SColor &in pixel, int offset) override
	{
		...
		switch (pixel.color)
		{
			...
			case tc_colors::color_oilrig_neutral:
			{
				autotile(offset);
				spawnBlob(map, "oilrig", offset, -1);
				break;
			}
			...
			case tc_colors::color_fort_blue:
			{
				spawnBlob(map, "camp", offset, 0);
				break;
			}
			case tc_colors::color_fort_red:
			{
			...
			case tc_colors::color_bricks:
			{
				map.SetTile(offset, CMap::tile_bricks);
				break;
			}
			case tc_colors::color_titanium:
			{
				map.SetTile(offset, CMap::tile_titanium);
				break;
			}
...
void CalculateMinimapColour(CMap@ this, u32 offset, TileType type, SColor &out col)
{
	...
	if (!air)
	{
		...
		switch(type)
		{
			...
			// BRICKS
			case CMap::tile_bricks:
			case CMap::tile_bricks_v0:
			case CMap::tile_bricks_v1:
			case CMap::tile_bricks_v2:
			case CMap::tile_bricks_v3:
			case CMap::tile_bricks_v4:
			case CMap::tile_bricks_v5:
			case CMap::tile_bricks_v6:
			case CMap::tile_bricks_v7:
			case CMap::tile_bricks_v8:
			case CMap::tile_bricks_v9:
			case CMap::tile_bricks_v10:
			case CMap::tile_bricks_v11:
			case CMap::tile_bricks_v12:
			case CMap::tile_bricks_v13:
			case CMap::tile_bricks_v14:
			case CMap::tile_bricks_d0:
			case CMap::tile_bricks_d1:
			case CMap::tile_bricks_d2:
			...
			case CMap::tile_titanium_v2:
			case CMap::tile_titanium_v3:
			case CMap::tile_titanium_v4:
			case CMap::tile_titanium_v5:
			case CMap::tile_titanium_v6:
			case CMap::tile_titanium_v7:
			case CMap::tile_titanium_v8:
			case CMap::tile_titanium_v9:
			case CMap::tile_titanium_v10:
			case CMap::tile_titanium_v11:
			case CMap::tile_titanium_v12:
			case CMap::tile_titanium_v13:
			case CMap::tile_titanium_v14:
			case CMap::tile_titanium_d0:
			case CMap::tile_titanium_d1:
			case CMap::tile_titanium_d2:
			case CMap::tile_titanium_d3:
			case CMap::tile_titanium_d4:
			case CMap::tile_titanium_d5:
			case CMap::tile_titanium_d6:
			case CMap::tile_titanium_d7:
				col = c_titanium;
			break;
...
TileType server_onTileHit(CMap@ map, f32 damage, u32 index, TileType oldTileType)
{
	if(map.getTile(index).type > 255)
	{
		switch(oldTileType)
		{
			...
			case CMap::tile_bricks_v13:
			case CMap::tile_bricks_v14:
			case CMap::tile_bricks_f14:
			{
				Vec2f pos = map.getTileWorldPosition(index);

				map.server_SetTile(pos, CMap::tile_bricks_d0);
				map.AddTileFlag(index, Tile::SOLID | Tile::COLLISION);
				map.RemoveTileFlag(index, Tile::LIGHT_PASSES | Tile::LIGHT_SOURCE);

				for (u8 i = 0; i < 4; i++)
				{
					bricks_Update(map, map.getTileWorldPosition(index) + directions[i]);
				}
				return CMap::tile_bricks_d0;
			}
			...
			case CMap::tile_titanium_v13:
			case CMap::tile_titanium_v14:
			case CMap::tile_titanium_f14:
			{
				Vec2f pos = map.getTileWorldPosition(index);

				map.server_SetTile(pos, CMap::tile_titanium_d0);
				map.AddTileFlag(index, Tile::SOLID | Tile::COLLISION);
				map.RemoveTileFlag(index, Tile::LIGHT_PASSES | Tile::LIGHT_SOURCE);

				for (u8 i = 0; i < 4; i++)
				{
					titanium_Update(map, map.getTileWorldPosition(index) + directions[i]);
				}
			...
			case CMap::tile_titanium_d7:
			{
				if (isClient())
				{
					...
					Sound::Play("dig_stone.ogg", pos, 1.1f, 0.89f);
						sparks(pos, 1, 1);
				}
				return CMap::tile_empty;
			}
	...
	if (map.getTile(index).type > 255)
	{
		...
		switch(tile_new)
		{
			case CMap::tile_glass:
			{
				...
				map.AddTileFlag(index, Tile::SOLID | Tile::COLLISION | Tile::LIGHT_PASSES | Tile::LIGHT_SOURCE);
...
void OnBricksTileHit(CMap@ map, u32 index)
{
	map.AddTileFlag(index, Tile::SOLID | Tile::COLLISION);
	map.RemoveTileFlag(index, Tile::LIGHT_PASSES);
...
void OnBricksTileDestroyed(CMap@ map, u32 index)
{
	if (isClient())
	{
		Vec2f pos = map.getTileWorldPosition(index);
...
void bricks_SetTile(CMap@ map, Vec2f pos)
{
	map.SetTile(map.getTileOffset(pos), CMap::tile_bricks + bricks_GetMask(map, pos));

	for (u8 i = 0; i < 4; i++)
	{
		bricks_Update(map, pos + directions[i]);
	}
}

u8 bricks_GetMask(CMap@ map, Vec2f pos)
{
	u8 mask = 0;

	for (u8 i = 0; i < 4; i++)
	{
		if (isbricksTile(map, pos + directions[i])) mask |= 1 << i;
	}

	return mask;
}

void bricks_Update(CMap@ map, Vec2f pos)
{
	u16 tile = map.getTile(pos).type;
	if (isbricksTile(map, pos))
		map.SetTile(map.getTileOffset(pos),CMap::tile_bricks+bricks_GetMask(map,pos));
}

bool isbricksTile(CMap@ map, Vec2f pos)
{
	u16 tile = map.getTile(pos).type;
...
void OnTitaniumTileDestroyed(CMap@ map, u32 index)
{
	if (isClient())
	{
		...
		ParticleAnimated("Smoke.png", pos+Vec2f(4, 0),
		Vec2f(0, 0), 0.0f, 1.0f, 3, 0.0f, false);
		Sound::Play("destroy_stone.ogg", pos, 1.1f, 0.95f);
	}
}

void titanium_SetTile(CMap@ map, Vec2f pos)
{
	map.SetTile(map.getTileOffset(pos), CMap::tile_titanium + titanium_GetMask(map, pos));
	map.SetTileSupport(map.getTileOffset(pos), 5);

	for (u8 i = 0; i < 4; i++)
	{
		titanium_Update(map, pos + directions[i]);
	}
}

u8 titanium_GetMask(CMap@ map, Vec2f pos)
{
	u8 mask = 0;

	for (u8 i = 0; i < 4; i++)
	{
		if (istitaniumTile(map, pos + directions[i])) mask |= 1 << i;
	}

	return mask;
}

void titanium_Update(CMap@ map, Vec2f pos)
{
	u16 tile = map.getTile(pos).type;
	if (istitaniumTile(map, pos))
		map.SetTile(map.getTileOffset(pos),CMap::tile_titanium+titanium_GetMask(map,pos));
}

bool istitaniumTile(CMap@ map, Vec2f pos)
{
	u16 tile = map.getTile(pos).type;
	return tile >= CMap::tile_titanium && tile <= CMap::tile_titanium_v14;
}
