...
{
	return this.hasTag("barrier") && getGameTime() < this.get_u32("warmup_time");
}

void onTick(CRules@ this)
{
	if (this.exists("warmup_time") && getGameTime() < this.get_u32("warmup_time"))
		if (getGameTime() % 30 == 0) this.SetGlobalMessage("WARMUP TIME ENDS IN: "+((this.get_u32("warmup_time") - getGameTime())/30));
	if (getGameTime() >= this.get_u32("warmup_time")) this.SetGlobalMessage(""); // else doesnt work idk why
...
{
	if (cmd == this.getCommandID("set_barrier"))
	{
		addBarrier();
		done_sync = false;
	}
	else if(cmd == this.getCommandID("set_barrier_pos"))
	{
		f32 x1, x2, y1, y2;
