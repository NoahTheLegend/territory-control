...
void onRender(CSprite@ this)
{
	...
	if (blob.isMyPlayer())
	{
		...
		if (!v_fastrender) RenderTeamInventoryHUD(blob);
...
void RenderTeamInventoryHUD(CBlob@ this)
{
	...
	if (playerTeam < 7)
	{
...

		{
			...
			{
				CBlob@ baseBlob = factionBases[i];
				if (baseBlob.getTeamNum() != playerTeam) continue;
				if (this.getDistanceTo(baseBlob) < 500.0f) closeEnough = true;
			}
