...
void onInit(CRules@ this)
{
	...
	if (!cfg.loadFile("SlurFilter.cfg") ||
	    !cfg.readIntoArray_string(word_replacements, "replacements"))
	{
		warning("Could not read chat filter configuration from SlurFilter.cfg");
	}

	if (word_replacements.length % 2 != 0)
	{
		warning("Could not read chat filter configuration: Expected 'swear; pairs, got " + word_replacements.length + " strings");
	}
}
...
string KidSafeText(const string &in textIn)
{
	...
	bool isVital = true;
	for (int i = 0; i < word_replacements.length - 1; i++)
	{
