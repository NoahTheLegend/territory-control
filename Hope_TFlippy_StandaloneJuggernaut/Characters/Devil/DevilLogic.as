...
void devil_actorlimit_setup(CBlob@ this)
{
	u16[] networkIDs;
	this.set("LimitedActors", networkIDs);
}

bool devil_has_hit_actor(CBlob@ this, CBlob@ actor)
{
	u16[]@ networkIDs;
...
u32 devil_hit_actor_count(CBlob@ this)
{
	u16[]@ networkIDs;
	this.get("LimitedActors", @networkIDs);
...
void devil_add_actor_limit(CBlob@ this, CBlob@ actor)
{
	this.push("LimitedActors", actor.getNetworkID());
}

void devil_clear_actor_limits(CBlob@ this)
{
...
void onInit(CBlob@ this)
{
	...
	devil.state = DevilStates::normal;
	devil.prevState = DevilStates::normal;
	devil.actionTimer = 0;
	devil.attackDelay = 0;

	this.set("DevilInfo", @devil);

	this.set_f32("gib health", 0.0f);

	devil_actorlimit_setup(this);
...
void onTick(CBlob@ this)
{
	...
	DevilInfo@ devil;
	if (!this.get("DevilInfo", @devil)) return;

	devil.prevState = devil.state;
	...
	if (devil.state == DevilStates::stun)
	{
		moveVars.jumpFactor = 0.0f;
		moveVars.walkFactor = 0.0f;
		devil.actionTimer = 0;
		devil.stun--;
		if (devil.stun <= 0) devil.state = DevilStates::normal;
	}
	else if (devil.state == DevilStates::normal)
	{
		//Normal
		if (devil.attackDelay > 0) devil.attackDelay--;
		else if (pressed_lmb)
		{
			devil.state = DevilStates::charging;
			devil.actionTimer = 0;
		}
		if (this.isKeyJustPressed(key_action2) && !this.hasTag("noLMB"))
		{
			...
			devil.attackDirection = dir;
			devil.attackAimPos = aimPos;
			devil.attackRot = angle;
			angle = (aimPos - pos).Angle();
			devil.attackTrueRot = angle;

			devil.wasFacingLeft = this.isFacingLeft();
			devil.state = DevilStates::grabbing;
			devil.actionTimer = 0;

			if (isClient()) Sound::Play("/ArgLong", pos);
		}
	}
	else if (devil.state == DevilStates::charging)
	{
		...
		if (devil.actionTimer >= DevilVars::chargeTime)
		{
			devil.state = DevilStates::chargedAttack;
			devil.actionTimer = 0;
			...
			Vec2f force = devil.attackDirection * this.getMass() * 3.0f;
			this.AddForce(force);
		}
	}
	else if (devil.state == DevilStates::chargedAttack)
	{
		//Attacking with the hammer
		moveVars.jumpFactor *= attackJumpFactor;
		moveVars.walkFactor *= attackWalkFactor;
		this.SetFacingLeft(devil.wasFacingLeft);

		if (devil.actionTimer >= DevilVars::attackTime)
		{
			devil.state = DevilStates::normal;
			devil.actionTimer = 0;
			devil.attackDelay = DevilVars::attackDelay;
		}
		else if (devil.actionTimer < 12) DoAttack(this, 8.0f, devil, 120.0f, HittersTC::hammer, devil.actionTimer);

		devil.actionTimer += 1;
	}
	else if (devil.state == DevilStates::grabbing)
	{
		...
		if (devil.actionTimer >= DevilVars::grabTime)
		{
			devil.state = DevilStates::grabbed;
			devil.actionTimer = 0;
			devil.attackDelay = DevilVars::attackDelay * 2;
		}
		else if (devil.actionTimer <= (DevilVars::grabTime / 4) * 3)
		{
			//Grab
			const float range = 28.0f; //36.0f originally
			f32 angle = devil.attackRot;
			Vec2f dir = devil.attackDirection;
	...
	else if (devil.state == DevilStates::grabbed)
	{
		...
		else if (pressed_lmb)
		{
			...
			devil.state = DevilStates::throwing;
			devil.actionTimer = 0;
			if (isClient()) Sound::Play("/ArgLong", pos);
		...
		else if (pressed_rmb && this.isOnGround() && this.isKeyJustPressed(key_action2))
		{
			...
			if (point !is null)
			{
				...
				if (attachedBlob !is null)
				{
					devil.state = DevilStates::fatality;
					devil.actionTimer = 0;
					devil.wasFacingLeft = this.isFacingLeft();
				}
			}
	...
	else if (devil.state == DevilStates::throwing)
	{
		//Throwing
		if (devil.actionTimer >= DevilVars::throwTime)
		{
			devil.state = DevilStates::normal;
			devil.actionTimer = 0;
			devil.attackDelay = DevilVars::attackDelay;
	...
	else if (devil.state == DevilStates::fatality)
	{
		//RMB Fatality
		moveVars.jumpFactor = 0.0f;
		moveVars.walkFactor = 0.0f;
		...
		if (point !is null)
		{
			...
			{
				attachedBlob.SetFacingLeft(this.isFacingLeft());
				if (devil.actionTimer == 26)
				{
					attachedBlob.Tag("dead");
					point.offset.y = 3.0f;
					point.offset.x = 16.0f;
				}
				if (devil.actionTimer == 46)
				{
		...
		if (isClient())
		{
			if (devil.actionTimer == 3) Sound::Play("ArgShort.ogg", pos, 1.0f);
			else if (devil.actionTimer == 20) Sound::Play("ArgLong.ogg", pos, 1.0f);
			else if (devil.actionTimer == 29)
			{
				ShakeScreen(6.0f, 5, this.getPosition());
				Sound::Play("FallOnGround.ogg", pos, 0.4f);
			}
			else if (devil.actionTimer == 45) ShakeScreen(25.0f, 6, pos);
			else if (devil.actionTimer == 46)
			{
			...
			else if (devil.actionTimer == 48) Sound::Play("Gore.ogg", pos, 1.0f);
		}
		if (devil.actionTimer >= DevilVars::fatalityTime)
		{
			devil.state = DevilStates::normal;
			devil.actionTimer = 0;
	...
	if (devil.state != DevilStates::charging && devil.state != DevilStates::chargedAttack)
	{
		if (isServer()) devil_clear_actor_limits(this);
	}
	...
	if (point !is null)
	{
		...
		if (attachedBlob !is null)
		{
			if (devil.state != DevilStates::grabbed && devil.state != DevilStates::fatality) 
			{
				devil.state = DevilStates::grabbed;
			}
		}
		else if (devil.state == DevilStates::grabbed) devil.state = DevilStates::normal;
	}
}
...
{
	if (isClient()) this.getSprite().PlaySound("Devil_Death.ogg", 1.00f, this.getSexNum() == 0 ? 1.0f : 2.0f);
	if (isServer()) server_CreateBlob("devilhammer", this.getTeamNum(), this.getPosition());
}

void DoAttack(CBlob@ this, f32 damage, DevilInfo@ info, f32 arcDegrees, u8 type, int deltaInt)
{
	...
	if (map.getHitInfosFromArc(pos, aimangle, arcDegrees, radius + attack_distance, this, @hitInfos))
	{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore && deltaInt <= DevilVars::attackTime - 9) // blob
			{
				//big things block attacks
				const bool large = b.hasTag("blocks sword") && !b.isAttached() && b.isCollidable();

				if (!canHit(this, b) || devil_has_hit_actor(this, b))
				{
					// no TK
