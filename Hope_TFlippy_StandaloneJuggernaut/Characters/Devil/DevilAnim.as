...
{
	addRunnerTextures(this, "devil", "Devil");
...
{
	ensureCorrectRunnerTexture(this, "devil", "Devil");
}
...
void onTick(CSprite@ this)
{
	...
	DevilInfo@ devil;
	if (!blob.get("DevilInfo", @devil)) return;
	...
	// animations
	bool grabbed = devil.state == DevilStates::grabbed;

	blob.Untag("attack head");
	blob.Untag("dead head");

	if (devil.state != DevilStates::fatality) this.SetOffset(Vec2f());

	if (devil.state == DevilStates::fatality)
	{
		if (devil.prevState != devil.state) this.SetFacingLeft(false);
